% FIGURE_8_HB_MASS_MIDDLE_EXPERIMENT (SCRIPT)
% Numerical experiment for Figure 8 (HB-splines, “center” refinement on a
% cube). Compares the standard GeoPDEs mass assembly with the low-rank (TT)
% hierarchical mass assembly on the unit cube with homogeneous Dirichlet BCs.
% The script sweeps spline degrees, refinement depths, and TT tolerances, and
% records wall-clock time, memory, problem size, operator-norm estimates, and
% the discrepancy ‖M_TT − M_ref‖ over interior DoFs. Results are saved as .mat
% files for plotting.
%
% Purpose
% -------
% Reproduce the HB-spline mass-matrix curves in Figure 8 (cube geometry) by:
% • building an HB (non-truncated) hierarchy via a deterministic *central box*
%   refinement pattern,
...
% • measuring assembly time, TT memory vs. sparse memory, operator norms,
% • estimating ‖M_full − M_ref‖ via NORMEST on interior DoFs,
% • storing all metrics per (degree, level, tol) for later plotting.
%
% Problem setup
% -------------
% Geometry   : unit cube (problem_data.geo_name = 'geo_cube.txt').
...
% (A manufactured u_ex is defined but never evaluated here.)
%
...
% Base mesh  : nsub_coarse = [4, 4, 4]·p (isotropic coarse grid).
...
%
% Refinement pattern (central box)
% --------------------------------
% At each refinement round i_ref on the current coarsest active level:
% • Mark a *centered* rectangular box whose half-width in each direction is
%   floor(nel_dir(d)/(2^i_ref)), i.e., indices
%     d_center − half_width : d_center + half_width
%   where d_center = nel_dir(d)/2 in each direction d = 1,2,3.
% • Enforce admissibility (MARK_ADMISSIBLE), refine the mesh (HMSH_REFINE),
%   and update the hierarchical space (HSPACE_REFINE). HB (not truncated) is used.
%
...
%   2) Perform ‘it’ refinement rounds using the *central box* pattern (with
%      admissibility): marked → MARK_ADMISSIBLE → HMSH_REFINE → HSPACE_REFINE.
...
%        Save to 'hb_mass_cube_middle_gp.mat'.
...
%            Save to 'hb_mass_cube_middle_1.mat'.
%        (b) block_format = 0: repeat assembly & recording for per-level blocks.
%            Save to 'hb_mass_cube_middle_2.mat'.
%
% Outputs (saved to disk)
% -----------------------
% • 'hb_mass_cube_middle_gp.mat'
...
%
% • 'hb_mass_cube_middle_1.mat'  (TT, block_format = 1)
...
%
% • 'hb_mass_cube_middle_2.mat'  (TT, block_format = 0)
%   results_2 struct (same fields as results_1 above, for per-level blocks).
%
% How it works (high level)
% -------------------------
% 1) Set up the cube problem with Dirichlet BCs and c_diff = 1.
% 2) For each degree p:
%    a) Build HB hierarchies up to levels(p) with the *central box* marking
%       pattern at each refinement (with admissibility).
...
for i_deg = 1:degrees_n
    ...
    for it = 0:(levels(i_deg)-1)
        ...
        save('hb_mass_cube_middle_gp.mat','results');
        ...
        for i_tol = 1:tol_n
            ...
            save('hb_mass_cube_middle_1.mat','results_1');
            ...
            save('hb_mass_cube_middle_2.mat','results_2');
