% FIGURE_5_HB_MASS_FLAG_EXPERIMENT (SCRIPT)
% Numerical experiment for Figure 5 (HB-splines, 3D-flag geometry).
% Compares standard GeoPDEs mass assembly vs. low-rank (TT) hierarchical
% assembly on a complex NURBS “flag” patch with homogeneous Dirichlet BCs.
% Sweeps spline degrees, refinement depths, and TT tolerances; records
% wall-clock time, memory, problem size, operator norm estimates, and the
% discrepancy ‖M_TT - M_ref‖ over interior DoFs. Results are saved to .mat
% files for plotting the figure in the paper.
%
% Purpose
% -------
% Reproduce the HB-spline mass-matrix curves in Figure 5 (flag geometry) by:
% • building an HB (non-truncated) hierarchy via a deterministic x-slab
%   refinement pattern,
% • assembling the reference mass matrix with OP_U_V_HIER,
% • assembling the low-rank hierarchical mass with ASSEMBLE_MASS_LOW_RANK
%   (block format = per active cuboid) and materializing M_full,
...
%       - Initialize (hmsh, hspace, geometry) on the coarse mesh (HB).
...
%             MARK_ADMISSIBLE → HMSH_REFINE → HSPACE_REFINE.
...
% 2) For each degree:
%    a) Set HB parameters and build the hierarchy to ‘levels(p)’ with slab marking.
...
%   but is not defined—harmless here since u_ex is not evaluated.
%
...
for i_deg = 1:degrees_n
    ...
    for it = 0:(levels(i_deg)-1)
        ...
        save('hb_mass_flag_1_bslice_gp.mat','results');
        ...
        for i_tol = 1:tol_n
            ...
            save('hb_mass_flag_1_bslice_1.mat','results_1');
