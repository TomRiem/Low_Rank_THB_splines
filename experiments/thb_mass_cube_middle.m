% FIGURE_8_THB_MASS_MIDDLE_EXPERIMENT (SCRIPT)
% Numerical experiment for Figure 8 (THB-splines, “center” refinement on a
% cube). Compares the standard GeoPDEs mass assembly with the low-rank (TT)
% hierarchical mass assembly. The script sweeps spline degrees, refinement
% depths, and TT tolerances, and logs wall-clock time, memory, problem size,
% operator-norm estimates, and the discrepancy ‖M_TT − M_ref‖ on interior DoFs.
% Results are saved as .mat files for plotting.
%
% Purpose
% -------
% Reproduce the THB-spline mass-matrix curves in Figure 8 (cube geometry) by:
% • building a THB (truncated) hierarchy via a deterministic *central box*
%   refinement pattern,
...
% Base mesh  : nsub_coarse = [2,2,2]·p + [2,2,2]  (slightly denser than 2p).
...
%
% Refinement pattern (central box, THB)
% -------------------------------------
% At each refinement round i_ref on the current coarsest active level:
% • Mark a *centered* rectangular box of element indices
%     (d_center − p + 1) : (d_center + p)
%   in each direction d = 1,2,3, where d_center = nel_dir(d)/2.
%   (Half-width p per direction → ~2p elements wide.)
% • Enforce admissibility (MARK_ADMISSIBLE), refine mesh (HMSH_REFINE),
%   deactivate/activate basis functions (COMPUTE_FUNCTIONS_TO_DEACTIVATE),
%   and update the hierarchical space (HSPACE_REFINE).
% • Truncation is active, so parent functions are truncated over refined regions.
%
...
%   2) Perform ‘it’ refinement rounds using the *central box* pattern
%      (with admissibility and truncation).
...
%        Save to 'thb_mass_cube_middle_gp.mat'.
...
%            Save to 'thb_mass_cube_middle_1.mat'.
%        (b) block_format = 0: repeat assembly & recording for per-level blocks.
%            Save to 'thb_mass_cube_middle_2.mat'.
%
% Outputs (saved to disk)
% -----------------------
% • 'thb_mass_cube_middle_gp.mat'
...
%
% • 'thb_mass_cube_middle_1.mat'  (TT, block_format = 1)
...
%
% • 'thb_mass_cube_middle_2.mat'  (TT, block_format = 0)
...
% 2) For each degree p:
%    a) Build THB hierarchies up to levels(p) with the *central box* marking
%       pattern at each refinement (with admissibility and truncation).
...
%   COMPUTE_FUNCTIONS_TO_DEACTIVATE, HSPACE_REFINE, ADAPTIVITY_INTERPOLATION_SYSTEM,
...
for i_deg = 1:degrees_n
    ...
    for it = 0:(levels(i_deg)-1)
        ...
        save('thb_mass_cube_middle_gp.mat','results');
        ...
        for i_tol = 1:tol_n
            ...
            save('thb_mass_cube_middle_1.mat','results_1');
            ...
            save('thb_mass_cube_middle_2.mat','results_2');
