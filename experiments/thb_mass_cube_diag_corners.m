% FIGURE_6_THB_MASS_CUBE_DIAG_CORNERS_EXPERIMENT (SCRIPT)
% Numerical experiment for Figure 6 (THB-splines, cube geometry, two opposite
% corner refinements). Compares the standard GeoPDEs mass assembly with the
% low-rank (TT) hierarchical assembly on a unit cube with homogeneous Dirichlet
% BCs. The script sweeps spline degrees, refinement depths, and TT tolerances,
% and records wall-clock time, memory, problem size, operator-norm estimates,
% and the discrepancy ‖M_TT − M_ref‖ over interior DoFs. Results are saved to
% .mat files for the plots in the paper.
%
% Purpose
% -------
% Reproduce the THB-spline mass-matrix curves in Figure 6 (cube geometry) by:
...
% Base mesh  : nsub_coarse = [2, 2, 2]*p + 1 (uniform, degree-scaled).
% Refinement : dyadic, nsub_refine = [2, 2, 2].
% Quadrature : nquad = [5, 5, 5] for standard GeoPDEs assembly.
% Admissibility: adaptivity_data.adm_class = 2 (H-admissible).
%
% Refinement pattern (two opposite corners)
% -----------------------------------------
% At each refinement round i_ref on the current coarsest active level:
% • Mark a p×p×p block of cells at the “lower” corner (indices 1…p in each dir).
% • Mark a p×p×p block at the “upper” corner (last p indices in each dir).
% • Enforce admissibility, refine the mesh (HMSH_REFINE), and refine the
%   hierarchical space (HSPACE_REFINE). Truncation is automatically applied.
%
...
%        marked → MARK_ADMISSIBLE → HMSH_REFINE → HSPACE_REFINE (truncated).
...
% -----------------------
% • 'thb_mass_cube_diag_corners_gp.mat'
...
%
% • 'thb_mass_cube_diag_corners_1.mat'  (TT, block_format = 1)
...
%
% • 'thb_mass_cube_diag_corners_2.mat'  (TT, block_format = 0)
...
%       by fixed p-sized blocks on each level (with admissibility).
...
for i_deg = 1:degrees_n
    ...
    method_data.nsub_coarse = [2 2 2]*p + 1;
    ...
    for it = 0:(levels(i_deg)-1)
        ...
        save('thb_mass_cube_diag_corners_gp.mat','results');
        ...
        for i_tol = 1:tol_n
            ...
            save('thb_mass_cube_diag_corners_1.mat','results_1');
            ...
            save('thb_mass_cube_diag_corners_2.mat','results_2');
