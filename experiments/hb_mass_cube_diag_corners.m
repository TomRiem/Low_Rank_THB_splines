% FIGURE_6_HB_MASS_CUBE_DIAG_CORNERS_EXPERIMENT (SCRIPT)
% Numerical experiment for Figure 6 (HB-splines, cube geometry, two opposite
% corner refinements). Compares standard GeoPDEs mass assembly vs. low-rank
% (TT) hierarchical assembly on a unit cube with homogeneous Dirichlet BCs.
% The script sweeps spline degrees, refinement depths, and TT tolerances; it
% records wall-clock time, memory, problem size, operator-norm estimates, and
% the discrepancy ‖M_TT − M_ref‖ over interior DoFs. Results are saved to
% .mat files for figure generation.
%
% Purpose
% -------
% Reproduce the HB-spline mass-matrix curves in Figure 6 (cube geometry) by:
% • building an HB (non-truncated) hierarchy via a deterministic “two opposite
%   corners” refinement pattern,
...
% Problem setup
% -------------
% Geometry   : 'geo_cube.txt' (NURBS map of the cube).
...
% Base mesh  : nsub_coarse = [4 4 4]*p + 1 (uniform, degree-scaled).
...
%
% Refinement pattern (Figure 6)
% -----------------------------
% At each refinement round i_ref on the current coarsest active level:
% • Mark all cells in two opposite corner subcubes:
%   - a “lower” corner block of size ⌊nel_dir / 2^{i_ref}⌋ in each direction,
%   - an “upper” corner block of the same size at the far (+,+,+) corner.
% • Enforce admissibility, refine mesh (HMSH_REFINE), and refine the
%   hierarchical space (HSPACE_REFINE) accordingly.
%
...
%        marked → MARK_ADMISSIBLE → HMSH_REFINE → HSPACE_REFINE.
...
%        Record: assembly time, memory of mass_mat(int,int), ndof, NORMEST(mass_mat(int,int)).
...
% -----------------------
% • 'hb_mass_cube_diag_corners_gp.mat'
...
%
% • 'hb_mass_cube_diag_corners_1.mat'  (TT, block_format = 1)
...
%
% • 'hb_mass_cube_diag_corners_2.mat'  (TT, block_format = 0)
%   results_2 struct (same fields as results_1 above, for per-level blocks).
%
% How it works (high level)
% -------------------------
% 1) Set Dirichlet BCs and c_diff = 1 on the cube geometry.
% 2) For each degree p:
...
for i_deg = 1:degrees_n
    ...
    method_data.nsub_coarse = [4 4 4]*p + 1;
    ...
    for it = 0:(levels(i_deg)-1)
        ...
        save('hb_mass_cube_diag_corners_gp.mat','results');
        ...
        for i_tol = 1:tol_n
            ...
            save('hb_mass_cube_diag_corners_1.mat','results_1');
            ...
            save('hb_mass_cube_diag_corners_2.mat','results_2');
