% FIGURE_9_HB_MASS_FLAG_MIDDLE_EXPERIMENT (SCRIPT)
% Numerical experiment for Figure 9 (HB-splines on the “3D flag” geometry).
...
% Purpose
% -------
% Reproduce HB-spline mass-matrix curves on a nontrivial NURBS geometry:
% • build an HB hierarchy on the “3D flag” (loaded from 3Dflag_GeoPDEs.mat),
...
% Base mesh  : nsub_coarse = [2, 2, 4] · p  (anisotropic: z is denser).
...
%
% Refinement pattern (centered window, HB)
% ---------------------------------------
% At each refinement round i_ref on the current coarsest active level:
% • Mark a *centered* rectangular box of element indices with half-width
%   ~ floor(nel_dir/2^i_ref) around the mesh center in each direction.
%   (For i_ref = 1 this covers (almost) the whole domain; subsequent rounds
%    shrink the marked window dyadically around the center.)
...
% • HB hierarchy (no truncation) is maintained throughout.
%
...
%   1) Initialize (hmsh, hspace, geometry) on the HB coarse mesh of the flag.
%   2) Perform ‘it’ refinement rounds using the centered window (with
%      admissibility). HB (non-truncated) hierarchy is preserved.
...
%        Save to 'hb_mass_flag_middle_gp.mat'.
...
%            Save to 'hb_mass_flag_middle_1.mat'.
%        (b) block_format = 0: repeat assembly & recording for per-level blocks.
%            Save to 'hb_mass_flag_middle_2.mat'.
%
% Outputs (saved to disk)
% -----------------------
% • 'hb_mass_flag_middle_gp.mat'
...
%
% • 'hb_mass_flag_middle_1.mat'  (TT, block_format = 1)
...
%
% • 'hb_mass_flag_middle_2.mat'  (TT, block_format = 0)
...
% 2) For each degree p:
%    a) Build HB hierarchies up to levels(p) with the *centered* marking pattern
...
for i_deg = 1:degrees_n
    ...
    method_data.nsub_coarse = [2 2 4].*p;
    ...
    for it = 0:(levels(i_deg)-1)
        ...
        save('hb_mass_flag_middle_gp.mat','results');
        ...
        for i_tol = 1:tol_n
            ...
            save('hb_mass_flag_middle_1.mat','results_1');
            ...
            save('hb_mass_flag_middle_2.mat','results_2');
