% FIGURE_4_THB_MASS_EXPERIMENT (SCRIPT)
% Numerical experiment for Figure 4 (THB-splines, mass matrix assembly).
% Compares standard GeoPDEs mass assembly vs. low-rank (TT) hierarchical
% assembly on a 3D cube with homogeneous Dirichlet boundary conditions.
% Sweeps over spline degrees, refinement depths, and TT tolerances, and
% records timing, memory, size, norm estimates, and matrix discrepancies.
% Results are saved to .mat files for plotting the figure in the paper.
%
% Purpose
% -------
% Reproduce the THB-spline mass-matrix curves in Figure 4 by:
% • building a THB hierarchy via a deterministic x-slab refinement pattern,
% • assembling the reference (standard) mass matrix with OP_U_V_HIER,
% • assembling the low-rank hierarchical mass matrix with ASSEMBLE_MASS_LOW_RANK
%   in block format (per-cuboid), materializing M_full for comparisons,
% • measuring assembly time, memory (TT size vs. sparse), operator norms,
% • estimating the matrix discrepancy ‖M_TT - M_ref‖ via NORMEST,
% • storing all metrics per (degree, level, tol) for later plotting.
%
% Problem setup
% -------------
% Geometry   : 'geo_cube.txt' (NURBS map, GeoPDEs format), Ω = [0,1]^3.
% BCs        : Dirichlet on all 6 faces; no Neumann sides.
% Coefficient: c_diff ≡ 1.
% Boundary data handle h is zero; no load vector is needed for mass tests.
% (A manufactured u_ex is defined but not used in this script.)
%
...
% Base mesh  : nsub_coarse = [2*p, 2, 2].
% Refinement : dyadic nsub_refine = [2,2,2].
% Quadrature : nquad = [5, 5, 5] for standard GeoPDEs assembly.
% Admissibility: adaptivity_data.adm_class = 2 (used in MARK_ADMISSIBLE).
%
...
%             for all y,z indices; enforce admissibility; refine mesh/space.
%       - Reference mass (standard GeoPDEs):
%           * mass_mat = OP_U_V_HIER(hspace, hspace, hmsh, c_diff)
%           * Apply Dirichlet elimination via SP_DRCHLT_L2_PROJ, define
%             int_dofs = setdiff(1:ndof, dirichlet_dofs).
%           * Record time_assemble (time_gp), memory of mass_mat(int,int),
...
%           * Record time_assemble (t_assemble), TT memory (RecursiveSize(TT_M)),
%             and discrepancy err = NORMEST(M_full(int,int) - mass_mat(int,int)).
%
% Outputs (saved to disk)
% -----------------------
% • 'thb_mass_cube_1_bslice_gp.mat'
...
% -------------------------
% 1) Initialize RNG and problem data (cube geometry, Dirichlet BCs, c_diff=1).
% 2) For each degree:
%    a) Set THB parameters, build hierarchy up to ‘levels(p)’ via slab marking.
%    b) Assemble the reference mass matrix with OP_U_V_HIER; time & measure it.
%    c) For each TT tolerance:
%       i)  Interpolate geometry weights to TT (H), time the interpolation.
%       ii) Assemble hierarchical TT mass (per-cuboid), time the assembly.
%       iii) Compare M_full against the reference submatrix (interior DoFs)
%            with NORMEST; record TT memory size.
% 3) Save results after each step to the .mat files named above.
%
% Notes & dependencies
% --------------------
...
% • The u_ex handle is defined but not used; note that the symbol ‘c’ is not
%   defined in that expression here—this is harmless since u_ex is unused.
%
...
for i_deg = 1:degrees_n
    ...
    for it = 0:(levels(i_deg)-1)
        ...
        save('thb_mass_cube_1_bslice_gp.mat','results');
        ...
        for i_tol = 1:tol_n
            ...
            save('thb_mass_cube_1_bslice_1.mat','results_1');
