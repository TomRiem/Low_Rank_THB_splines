% FIGURE_4_HB_MASS_EXPERIMENT (SCRIPT)
% Numerical experiment for Figure 4 (HB-splines, mass matrix assembly).
...
% Purpose
% -------
% Reproduce the HB-spline mass-matrix curves in Figure 4 by:
% • building a hierarchy via a deterministic x-slab refinement pattern,
...
%       - Initialize (hmsh, hspace, geometry) on the coarse mesh.
...
% 2) For each degree:
%    a) Set HB parameters, build hierarchy up to ‘levels(p)’ via slab marking.
...
% • The manufactured u_ex is defined but not used (mass test only).
%
...
for i_deg = 1:degrees_n
    ...
    for it = 0:(levels(i_deg)-1)
        ...
        save('hb_mass_cube_1_bslice_gp.mat','results');
        ...
        for i_tol = 1:tol_n
            ...
            save('hb_mass_cube_1_bslice_1.mat','results_1');
