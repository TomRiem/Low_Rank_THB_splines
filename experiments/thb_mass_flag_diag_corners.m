% FIGURE_7_THB_MASS_FLAG_DIAG_CORNERS_EXPERIMENT (SCRIPT)
% Numerical experiment for Figure 7 (THB-splines, “flag” geometry, two
...
% Purpose
% -------
% Reproduce the THB-spline mass-matrix curves in Figure 7 (flag geometry) by:
% • building a THB (truncated) hierarchy via a deterministic “two opposite
%   corners” refinement pattern on the flag,
...
% Base mesh  : nsub_coarse = [p+1, p+1, 2p+1] (anisotropic coarse grid).
...
% • Mark a p×p×p corner block at the “origin” (indices 1…p in each dir.).
% • Mark the symmetric p×p×p block at the far corner (last p indices).
% • Enforce admissibility (MARK_ADMISSIBLE), refine the mesh (HMSH_REFINE),
%   and update the hierarchical space with truncation (HSPACE_REFINE). THB is used.
%
...
%   1) Initialize (hmsh, hspace, geometry) on the THB coarse mesh.
%   2) Perform ‘it’ refinement rounds using the two-corner p×p×p pattern:
%        marked → MARK_ADMISSIBLE → HMSH_REFINE → HSPACE_REFINE (THB).
...
%        Save to 'thb_mass_flag_diag_corners_gp.mat'.
...
%            Save to 'thb_mass_flag_diag_corners_1.mat'.
%        (b) block_format = 0: repeat assembly & recording for per-level blocks.
%            Save to 'thb_mass_flag_diag_corners_2.mat'.
%
% Outputs (saved to disk)
% -----------------------
% • 'thb_mass_flag_diag_corners_gp.mat'
...
%
% • 'thb_mass_flag_diag_corners_1.mat'  (TT, block_format = 1)
...
%
% • 'thb_mass_flag_diag_corners_2.mat'  (TT, block_format = 0)
...
% 2) For each degree p:
%    a) Build THB hierarchies up to levels(p), refining two opposite corners
%       with p×p×p marked blocks at every refinement (with admissibility).
...
for i_deg = 1:degrees_n
    ...
    method_data.nsub_coarse = [p+1 p+1 2*p+1];
    ...
    for it = 0:(levels(i_deg)-1)
        ...
        for i_ref = 1:it
            ...
            for k = (hmsh.mesh_of_level(i_ref).nel_dir(3)- p + 1):hmsh.mesh_of_level(i_ref).nel_dir(3)
                for j = (hmsh.mesh_of_level(i_ref).nel_dir(2)- p + 1):hmsh.mesh_of_level(i_ref).nel_dir(2)
                    for i = (hmsh.mesh_of_level(i_ref).nel_dir(1)- p + 1):hmsh.mesh_of_level(i_ref).nel_dir(1)
                        marked{i_ref} = [marked{i_ref}; sub2ind(hmsh.mesh_of_level(i_ref).nel_dir, i, j, k)];
                    end
                end
            end
        ...
        save('thb_mass_flag_diag_corners_gp.mat','results');
        ...
        for i_tol = 1:tol_n
            ...
            save('thb_mass_flag_diag_corners_1.mat','results_1');
            ...
            save('thb_mass_flag_diag_corners_2.mat','results_2');
