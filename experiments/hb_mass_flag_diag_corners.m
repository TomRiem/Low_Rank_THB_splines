% FIGURE_7_HB_MASS_FLAG_DIAG_CORNERS_EXPERIMENT (SCRIPT)
% Numerical experiment for Figure 7 (HB-splines, “flag” geometry, two
% opposite corner refinements). Compares standard GeoPDEs mass assembly with
% the low-rank (TT) hierarchical assembly on a NURBS flag model with
% homogeneous Dirichlet BCs. The script sweeps spline degrees, refinement
% depths, and TT tolerances, and records wall-clock time, memory, problem
% size, operator-norm estimates, and the discrepancy ‖M_TT − M_ref‖ over
% interior DoFs. Results are saved to .mat files for plotting.
%
% Purpose
% -------
% Reproduce the HB-spline mass-matrix curves in Figure 7 (flag geometry) by:
% • building an HB (non-truncated) hierarchy via a deterministic
%   “two opposite corners” refinement pattern on the flag,
% • assembling the reference mass with OP_U_V_HIER,
% • assembling the low-rank hierarchical mass with ASSEMBLE_MASS_LOW_RANK
%   in both TT block layouts (per-cuboid and per-level),
...
% Problem setup
% -------------
% Geometry   : load('3Dflag_GeoPDEs.mat'); problem_data.geo_name = g.nurbs.
...
% -----------------------
% Trial/Test : Hierarchical B-splines (HB) → method_data.truncated = 0.
% Degrees    : p ∈ {3, 5} (isotropic: [p p p]).
% Regularity : C^{p−1} in each direction.
% Base mesh  : nsub_coarse = [2, 2, 4]*p + 1 (anisotropic coarse grid).
...
%
% Refinement pattern (two opposite “diagonal corners” on the flag)
% ----------------------------------------------------------------
% At each refinement round i_ref on the current coarsest active level:
% • Mark the lower p×p×p octant-like block (indices 1…⌊nel_dir/2^{i_ref}⌋).
% • Mark the upper symmetric block (last ⌊nel_dir/2^{i_ref}⌋ indices).
% • Enforce admissibility (MARK_ADMISSIBLE), refine the mesh (HMSH_REFINE),
%   and update the hierarchical space (HSPACE_REFINE). HB is used (no truncation).
%
...
%   1) Initialize (hmsh, hspace, geometry) on the HB coarse mesh.
%   2) Perform ‘it’ refinement rounds using the two-corner pattern above:
%        marked → MARK_ADMISSIBLE → HMSH_REFINE → HSPACE_REFINE (HB).
...
%            Save to 'hb_mass_flag_diag_corners_1.mat'.
%        (b) block_format = 0: repeat assembly & recording for per-level blocks.
%            Save to 'hb_mass_flag_diag_corners_2.mat'.
%
% Outputs (saved to disk)
% -----------------------
% • 'hb_mass_flag_diag_corners_gp.mat'
...
%
% • 'hb_mass_flag_diag_corners_1.mat'  (TT, block_format = 1)
...
%
% • 'hb_mass_flag_diag_corners_2.mat'  (TT, block_format = 0)
%   results_2 struct (same fields as results_1 above, for per-level blocks).
%
% How it works (high level)
% -------------------------
% 1) Load the NURBS flag geometry and enforce Dirichlet BCs with c_diff = 1.
% 2) For each degree p:
%    a) Build HB hierarchies up to levels(p), refining two opposite corners
%       by successively halved blocks on each level (with admissibility).
...
%       ii) Assemble hierarchical TT mass (first per-cuboid, then per-level),
%           materialize M_full (time t_assemble), compare interior blocks with
%           NORMEST, and record TT memory.
...
%   but is not defined—harmless here since u_ex is never evaluated.
%
...
for i_deg = 1:degrees_n
    ...
    method_data.nsub_coarse = [2 2 4]*p + 1;
    ...
    for it = 0:(levels(i_deg)-1)
        ...
        for i_ref = 1:it
            marked = cell(i_ref,1);
            marked{i_ref} = []; 
            for k = 1:floor(hmsh.mesh_of_level(i_ref).nel_dir(3)./(2^(i_ref)))
                for j = 1:floor(hmsh.mesh_of_level(i_ref).nel_dir(2)./(2^(i_ref)))
            ...
            for k = (hmsh.mesh_of_level(i_ref).nel_dir(3)- floor(hmsh.mesh_of_level(i_ref).nel_dir(3)./(2^(i_ref))) + 1):hmsh.mesh_of_level(i_ref).nel_dir(3)
                for j = (hmsh.mesh_of_level(i_ref).nel_dir(2)- floor(hmsh.mesh_of_level(i_ref).nel_dir(2)./(2^(i_ref))) + 1):hmsh.mesh_of_level(i_ref).nel_dir(2)
                    for i = (hmsh.mesh_of_level(i_ref).nel_dir(1)- floor(hmsh.mesh_of_level(i_ref).nel_dir(1)./(2^(i_ref))) + 1):hmsh.mesh_of_level(i_ref).nel_dir(1)
                        marked{i_ref} = [marked{i_ref}; sub2ind(hmsh.mesh_of_level(i_ref).nel_dir, i, j, k)];
                    end
                end
            end
        ...
        save('hb_mass_flag_diag_corners_gp.mat','results');
        ...
        for i_tol = 1:tol_n
            ...
            save('hb_mass_flag_diag_corners_1.mat','results_1');
            ...
            save('hb_mass_flag_diag_corners_2.mat','results_2');
