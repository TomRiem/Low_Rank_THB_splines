function [TT_M] = assemble_mass_level_nurbs_1(H, Tweights, level, level_ind, cuboid_cells, cuboid_splines_level, hspace, hmsh, low_rank_data)
% ASSEMBLE_MASS_LEVEL_NURBS_1
...
%
% TT_M = ASSEMBLE_MASS_LEVEL_NURBS_1(H, Tweights, level, level_ind, ...
...
% hierarchical level of a NURBS space. Integration is restricted to the
% active cell cuboids of that level (“_1” variant) and correctly accounts for
% NURBS rational weights via the provided univariate weight tensors.
%
% Inputs
% ------
...
% localized to this knot-area (returned inside H_plus.mass.M{d}); this call
% uses Tweights to incorporate the rational weights.
...
% (basis-change and global packing) place these blocks into the global system.
    ...
    for i_domain = 1:cuboid_cells{level_ind}.n_active_cuboids
        ...
        H_plus = univariate_u_v_area_nurbs(H_plus, hspace, level, level_ind, knot_area, cuboid_splines_level, Tweights);
        for i=1:H.mass.R(1)
            for j = 1:H.mass.R(3)
