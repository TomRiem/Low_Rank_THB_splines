function [u_full, u_tt, TT_K, TT_rhs, time, td, K_full] = hierarchical_solve_laplace_low_rank(H, rhs, hmsh, hspace, low_rank_data)
% hierarchical_solve_laplace_low_rank
...
%   hierarchical_solve_laplace_low_rank(H, rhs, hmsh, hspace, low_rank_data)
%
% Purpose
% -------
...
%   5: diagonals of option 3.
    ...
    for i_lev = 1:hspace.nlevels
        if isempty(hspace.active{i_lev}) 
            l_d = [l_d, i_lev];
        end
    end
    level(l_d) = [];
    ...
    if strcmp(hspace.space_of_level(1).space_type, 'spline')
        if (hspace.truncated)
            for i_lev = 1:nlevels
                ...
                if (cuboid_cells{i_lev}.n_active_cuboids <= cuboid_cells{i_lev}.n_not_active_cuboids + 1 && ...
                    ...
                    [TT_K{i_lev,i_lev}, TT_rhs{i_lev}] = assemble_stiffness_rhs_level_bsplines_1(H, rhs, level(i_lev), i_lev, ...
                        cuboid_cells, cuboid_splines_level, hspace, hmsh, low_rank_data);
                else
                    [TT_K{i_lev,i_lev}, TT_rhs{i_lev}] = assemble_stiffness_rhs_level_bsplines_2(H, rhs, level(i_lev), i_lev, ...
                        cuboid_cells, cuboid_splines_level, hspace, hmsh, low_rank_data);
                end
                if isfield(low_rank_data,'preconditioner') && (low_rank_data.preconditioner == 3 || low_rank_data.preconditioner == 5)
                    precon.K{i_lev} = TT_K{i_lev,i_lev};
                end
                ...
                for j_lev = (i_lev-1):-1:1
                    ...
                    TT_rhs{j_lev} = round(TT_rhs{j_lev} + CT_b, low_rank_data.rankTol_f);
                ...
                clear CT_K_C K_C CT_b TT_K_tmp
            end
            clear C
        else
            for i_lev = 1:nlevels
        ...
        if (hspace.truncated)
            for i_lev = 1:nlevels
                ...
                if (cuboid_cells{i_lev}.n_active_cuboids <= cuboid_cells{i_lev}.n_not_active_cuboids + 1 && ...
                        cuboid_cells{i_lev}.n_active_cuboids ~= 0) || cuboid_cells{i_lev}.n_not_active_cuboids == 0
                    [TT_K{i_lev,i_lev}, TT_rhs{i_lev}] = assemble_stiffness_rhs_level_nurbs_1(H, rhs, Tweights, level(i_lev), i_lev, ...
                        cuboid_cells, cuboid_splines_level, hspace, hmsh, low_rank_data);
                else
                    [TT_K{i_lev,i_lev}, TT_rhs{i_lev}] = assemble_stiffness_rhs_level_nurbs_2(H, rhs, Tweights, level(i_lev), i_lev, ...
                        cuboid_cells, cuboid_splines_level, hspace, hmsh, low_rank_data);
                end
                if isfield(low_rank_data,'preconditioner') && (low_rank_data.preconditioner == 3 || low_rank_data.preconditioner == 5)
                    precon.K{i_lev} = TT_K{i_lev,i_lev};
    ...
    if isfield(low_rank_data,'block_format') && all(low_rank_data.block_format == 1)
        [TT_K, TT_rhs, cuboid_splines_system, precon, low_rank_data] = assemble_system_rhs_precon_format_1(TT_K, TT_rhs, level, hspace, nlevels, cuboid_splines_level, precon, low_rank_data);
    else
        [TT_K, TT_rhs, cuboid_splines_system, precon, low_rank_data] = assemble_system_rhs_precon_format_2(TT_K, TT_rhs, level, hspace, nlevels, cuboid_splines_level, precon, low_rank_data);
    end
